(function(){
	...
	console.log(this.oSpans);
	//得到body部分的所有div
	this.oDivs = document.querySelector(bdSelector).querySelectorAll('div[name=hid]');
	console.log(this.oDivs);
	this.bindEvent();
}
Tab.prototype.bindEvent = function() {
	...
	for (var i = 0; i < self.oSpans.length; i++) {
	  ...
	  (function(i){
		//绑定鼠标移入事件
		self.oSpans[i].addEventListener('touchstart', function () {
		  ...
		  self.oSpans[i].className = 'all_order';
		  //给当前的span元素的类名 +是一个空格 current类名, 因为最后一个span默认有一个class为last, 添加后的为 last current。其他元素添加类名之后他的类名是 " current"
		  this.className += ' all_order'; //class的类名
		}) 
	  })(i);
	}
}
var teeb = new Tab("#nav_list", "#context");
})()
